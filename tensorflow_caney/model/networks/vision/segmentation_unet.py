...
def repeat_elem(tensor, rep):
    ...
    # by a factor of rep.
    # If tensor has shape (None,256,256,3), lambda returns a tensor of shape
...
def res_conv_block(x, filter_size, size, dropout, batch_norm=False):
    ...
    if batch_norm is True:
        conv = layers.BatchNormalization(axis=3)(conv)
    # Activation before addition with shortcut
    # conv = layers.Activation('relu')(conv)
    if dropout > 0:
        conv = layers.Dropout(dropout)(conv)
...
def gating_signal(input, out_size, batch_norm=False):
    ...
    :return: the gating feature map with the same dimension
    of the up layer feature map
    """
...
def UNet(input_shape, NUM_CLASSES=1, dropout_rate=0.0, batch_norm=True):
    ...
    up_16 = layers.UpSampling2D(
        size=(UP_SAMP_SIZE, UP_SAMP_SIZE), data_format="channels_last")(conv_8)
    ...
    up_32 = layers.UpSampling2D(
        size=(UP_SAMP_SIZE, UP_SAMP_SIZE), data_format="channels_last")(up_conv_16)
...
def Attention_UNet(input_shape, NUM_CLASSES=1, dropout_rate=0.0, batch_norm=True):
    ...
    up_64 = layers.UpSampling2D(
        size=(UP_SAMP_SIZE, UP_SAMP_SIZE), data_format="channels_last")(up_conv_32)
    ...
    up_128 = layers.UpSampling2D(
        size=(UP_SAMP_SIZE, UP_SAMP_SIZE), data_format="channels_last")(up_conv_64)
...
if __name__ == "__main__":
    ...
    simple_unet = Attention_UNet(
        (256, 256, 1), NUM_CLASSES=1, dropout_rate=0.0, batch_norm=True)
    simple_unet.summary()
